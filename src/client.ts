...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['BACKAI_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['BACKAI_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Backai API.
 */
export class Backai {
  apiKey: string;
   ...
   * @param {string} [opts.baseURL=process.env['BACKAI_BASE_URL'] ?? https://petstore3.swagger.io/api/v3] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('BACKAI_BASE_URL'),
    apiKey = readEnv('PETSTORE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.BackaiError(
        "The PETSTORE_API_KEY environment variable is missing or empty; either provide it, or instantiate the Backai client with an apiKey option, like new Backai({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Backai.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('BACKAI_LOG'), "process.env['BACKAI_LOG']", this) ??
      defaultLogLevel;
  ...
  static BackaiError = Errors.BackaiError;
...
export declare namespace Backai {
